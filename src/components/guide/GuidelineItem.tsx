      ...
      <section className="py-24 px-4" data-purpose="community-guidelines">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {GUIDELINE_ITEMS.map((item) => (
            <div
              key={item.id}
              className={`guide-guideline-card ${theme.card.base} ${theme.card.variants.default} ${theme.card.padding.lg} flex flex-col`}
            >
                ...
                {item.rules.map((rule, i) => (
                  <li key={i} className="flex gap-4">
                    ...
                    <p className="text-text-secondary text-sm leading-relaxed">
                      <strong className="text-text-primary">{rule.label}</strong> {rule.text}
                    </p>
                  </li>
                ))}
              </ul>
