...
```mermaid
graph TD
    subgraph FrontendApp ["프론트엔드 애플리케이션 (React + Vite + TS)"]
        Router[React Router]
        
        subgraph PagesLayer ["페이지 (View Layer)"]
            Home[홈]
            Community[국민 토론]
            AITalk[AI 토론 연습]
            Proposals["국민 제안 (UGC)"]
        ...
        subgraph LogicLayer ["기능 로직 (Business Logic Hooks)"]
            Auth[Auth Context]
        ...
        subgraph ServiceLayer ["서비스 및 데이터 영속성 계층 (Persistence Layer)"]
